...
void Generator::generateCSVSets(const char name[], int week, int tMax,
  ...
  for (int i = 4; i < 7; i++) {
    File << "Set " << i << " :," << checkForWeekAndReturnKG(week, i - 3, tMax)
         << ", x " << checkForWeekAndReturnSets(week, i - 3) << std::endl;
  }
}
...
void Generator::createCSVFile() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (tMax[i] == 0) {
      std::cerr << "Error! tMax's not initalized. Have you entered your maxes?"
                << std::endl;
      return;
    }
  }
